...
package object barneshut {
  ...
  case class Fork(nw: Quad, ne: Quad, sw: Quad, se: Quad) extends Quad {
    ...
    val massX: Float =
      if (mass == 0) centerX
      else (quads map { quad => quad.mass * quad.massX } sum) / mass

    val massY: Float =
      if (mass == 0) centerY
      else (quads map { quad => quad.mass * quad.massY } sum) / mass
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: Seq[Body]) extends Quad {
    ...
    val massX = bodies.foldLeft(0f) { (acc, b: Body) => acc + b.mass * b.x } / mass
    val massY = bodies.foldLeft(0f) { (acc, b: Body) => acc + b.mass * b.y } / mass
    ...
    def insert(b: Body): Quad = {
      ...
      if (size <= minimumSize) copy(bodies = newBodies)
        ...
        newBodies.foldLeft(Fork(nw, ne, sw, se)) { (acc, body) => acc.insert(body) }
      }
    }
  }
