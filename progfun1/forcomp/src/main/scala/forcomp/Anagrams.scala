...
object Anagrams {
  ...
  def wordOccurrences(word: Word): Occurrences = {
    val grouped = word.toLowerCase groupBy (char => char)
    val occ = grouped.map((t: (Char, String)) => (t._1, t._2.length)).toList.sortWith((a, b) => a._1 < b._1)
    occ
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    occurrences match {
      ...
      case head :: tail => {
        ...
        tailCombinations ++ (for (remained <- tailCombinations; index <- 1 to head._2) yield (head._1, index) :: remained)
      }
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    def updateList(accumulator: List[(Char, Int)], update: (Char, Int)): List[(Char, Int)] = {
      val filtered = y filter (tuple => tuple._1 == update._1)
      filtered match {
        case Nil => update :: accumulator
        case head :: Nil => accumulator
        case (sym, freq) :: tail => (sym, freq - update._2) :: accumulator
      }
    }

    x.foldLeft(List[(Char, Int)]())(updateList) sortWith ((a, b) => a._1 > a._2)
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def iterate(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(List())
        ...
        remained <- iterate(subtract(occurrences, wordOccurrences(word)))
        if comb.nonEmpty
      } yield word :: remained
    }
